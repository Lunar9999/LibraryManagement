...
def check_overdue_and_create_fine(borrow: Borrow, now: datetime = None, commit=True):
    """
    Checks if a borrowed item is overdue and creates or updates a fine if it is.
    Args:
    ...
    if borrow.due_date.date() < now.date():
        fine_amount = calculate_fine(date=borrow.due_date)

        # Check if a fine already exists for this borrow
        existing_fine = session.execute(
            select(md.Fine).where(md.Fine.borrow_id == borrow.id)  # SQLAlchemy 2.0+ syntax
        ).scalar()
        ...
        if existing_fine:
            ...
            stmt = (
                update(md.Fine)
                .where(md.Fine.borrow_id == borrow.id)
                .values(amount=fine_amount, date_created=now)
            )
