...
class Login(Resource):
    def post(self):
        ...
        stmt = select(
            md.UserAccount.id,  # Include the user ID in the query
        ...
        access_token = create_access_token(identity=user, additional_claims=additional_claims)  # Pass the user object
        refresh_token = create_refresh_token(identity=user, additional_claims=additional_claims)  # Pass the user object

        # Return the access token, refresh token, and user ID
        return jsonify(
